...
global NewSettings := JSON.stringify({"servername":"ps7", "firstrun":0, "user_id":0, "hash":0, "instance_id":0, "getdetailsonstart":0, "launchgameonstart":0, "alwayssavechests":1, "alwayssavecontracts":1, "alwayssavecodes":1, "NoSaveSetting":0});Server globals
global DummyData := "&language_id=1&timestamp=0&request_id=0&network_id=11&mobile_client_version=999"global CodestoEnter := "";User info globals
global UserID := 0global UserHash := 0global InstanceID := 0global UserDetails := []global ActiveInstance := 0global CurrentAdventure := ""global CurrentArea := ""global CurrentPatron := ""global CurrentChampions := ""global BackgroundAdventure := ""global BackgroundArea := ""global BackgroundPatron := ""global BackgroundChampions := ""global Background2Adventure := ""global Background2Area := ""global Background2Patron := ""global Background2Champions := ""global Background3Adventure := ""global Background3Area := ""global Background3Patron := ""global Background3Champions := ""global AchievementInfo := "This page intentionally left blank.`n`n`n`n`n`n`n"global BlessingInfo := "`n`n`n`n`n`n"global ChampDetails := ""global TotalChamps := 0;Inventory globals
global CurrentGems := ""global AvailableChests := ""global SpentGems := ""global CurrentGolds := ""global GoldPity := ""global CurrentSilvers := ""global CurrentTGPs := ""global AvailableTGs := ""global NextTGPDrop := ""global CurrentTokens := ""global CurrentTinyBounties := ""global CurrentSmBounties := ""global CurrentMdBounties := ""global CurrentLgBounties := ""global AvailableTokens := ""global AvailableFPs := ""global CurrentTinyBS := ""global CurrentSmBS := ""global CurrentMdBS := ""global CurrentLgBS := ""global CurrentHgBS := ""global AvailableBSLvs := "";Loot globals
global ChampionsUnlockedCount := 0global ChampionsActiveCount := 0global FamiliarsUnlockedCount := 0global CostumesUnlockedCount := 0global EpicGearCount := 0global BrivSlot4 := 0global BrivZone := 0;Modron globals
global FGCore := "`n`n"global BGCore := "`n`n"global BG2Core := "`n`n"global BG3Core := "`n`n";Patron globals
global MirtVariants := ""global MirtCompleted := ""global MirtVariantTotal := ""global MirtFPCurrency := ""global MirtChallenges := ""global MirtRequires := ""global MirtCosts := ""global VajraVariants := ""global VajraCompleted := ""global VajraVariantTotal := ""global VajraFPCurrency := ""global VajraChallenges := ""global VajraRequires := ""global VajraCosts := ""global StrahdVariants := ""global StrahdCompleted := ""global StrahdVariantTotal := ""global StrahdFPCurrency := ""global StrahdChallenges := ""global StrahdRequires := ""global StrahdCosts := ""global ZarielVariants := ""global ZarielCompleted := ""global ZarielVariantTotal := ""global ZarielFPCurrency := ""global ZarielChallenges := ""global ZarielRequires := ""global ZarielCosts := "";Web Tools globals
global WebToolFormation := "https://ic.byteglow.com/formation"global WebToolModron := "https://ic.byteglow.com/modron"global WebToolCodes := "https://incendar.com/idlechampions_codes.php#123";GUI globals
global oMyGUI := ""global OutputText := "Test"global OutputStatus := "Welcome to IdleCombos v" VersionNumberglobal CurrentTime := ""global CrashProtectStatus := "Crash Protect`nDisabled"global CrashCount := 0global LastUpdated := "No data loaded."global TrayIcon := systemroot "\system32\imageres.dll"global LastBSChamp := ""global foundCodeString := "";BEGIN:	default run commands
...
Get_Codes_Autoload()
{
    ...
    send, ^c
    ClipWait, 1
    if WinExist("IdleCombos")WinActivateif WinExist("Redeem Codes")WinActivatePaste()return}Get_Codes_Autoload_Run() {
    Get_Codes_Autoload()
    Redeem_Codes()
    return
}
...
ShowPityTimers()
{
    ...
    newestchamp := JSON.stringify(UserDetails.details.heroes[UserDetails.details.heroes.MaxIndex()].hero_id)newestchamp := StrReplace(newestchamp, """")chestsforepic := 1while (chestsforepic < 11) {
        if (chestsforepic == 1) {
...
ParseAdventureData()
{
    ...
    for k, v in UserDetails.details.modron_savesif (v.instance_id == ActiveInstance) {
        if (v.core_id == 1) {
            FGCore := "Core: Modest"
        }else if (v.core_id == 2) {
            FGCore := "Core: Strong"
...
ParseInventoryData()
{
    ...
    for k, v in UserDetails.details.buffsswitch v.buff_id {
...
ParsePatronData()
{
    for k, v in UserDetails.details.patronsswitch v.patron_id {
        case 1: {
            if v.unlocked == False {
                MirtVariants := "Locked"
            ...
            }else for kk, vv in v.progress_barsswitch vv.id {
            ...
            case 2: {
                ...
                {
                    TotalChamps := 0
                    for k, v in UserDetails.details.heroesif (v.owned == 1) {
                        TotalChamps += 1
                    }
                    ;
                    ChampDetails := ""
                ...
                CheckBlessings()
                {
                    ...
                    if (UserDetails.details.reset_upgrade_levels.44) { ;Helm-Slow and Steady (X Epics)epiccount := "Slow and Steady:`n    x" epicvalue " damage (" EpicGearCount " epics)`n"}championcount := ""championvalue := Round((1.02 ** ChampionsUnlockedCount), 2)if (UserDetails.details.reset_upgrade_levels.72) { ;Helm-Familiar Faces (X Champions)championcount := "Familiar Faces:`n    x" championvalue " damage (" ChampionsUnlockedCount " champions)`n"}championactivecount := ""championactivevalue := Round((1.02 ** ChampionsActiveCount), 2)if (UserDetails.details.reset_upgrade_levels.76) { ;Helm-Splitting the Party (X Champions)championactivecount := "Splitting the Party:`n    x" championactivevalue " damage (" ChampionsActiveCount " active champions)`n"}veterancount := ""veteranvalue := Round(1 + (0.1 * UserDetails.details.stats.completed_adventures_variants_and_patron_variants_c22), 2)if (UserDetails.details.reset_upgrade_levels.56) { ;Tiamat-Veterans of Avernus (X Adventures)veterancount := "Veterans of Avernus:`n    x" veteranvalue " damage (" UserDetails.details.stats.completed_adventures_variants_and_patron_variants_c22 " adventures)`n"}costumecount := ""costumevalue := Round((1.20 ** CostumesUnlockedCount), 2)if (UserDetails.details.reset_upgrade_levels.88) { ;Auril-Costume Party (X Skins)costumecount := "Costume Party:`n    x" costumevalue " damage (" CostumesUnlockedCount " skins)`n"}familiarcount := ""familiarvalue := Round((1.20 ** FamiliarsUnlockedCount), 2)if (UserDetails.details.reset_upgrade_levels.108) { ;Corellon-Familiar Stakes (X Familiars)familiarcount := "Familiar Stakes:`n    x" familiarvalue " damage (" FamiliarsUnlockedCount " familiars)`n"}BlessingInfo := "Blessing Details`n`n" epiccount championcount championactivecount veterancount costumecount familiarcountif (BlessingInfo == "Blessing Details`n`n") {
...
List_ChampIDs:
    {
        ...
        while (id < 121) {
            ...
            if (!mod(id, 4))champidlist := champidlist id ": " champname "`n"else
    ...
    if (campaignid == 17) {
        for k, v in UserDetails.details.game_instances {
            if (v.game_instance_id == ActiveInstance) {
                currenttimegate := JSON.stringify(v.defines.adventure_defines[1].requirements[1].champion_id)}}campaignid := KlehoFromID(currenttimegate)}else if !((campaignid < 3) or (campaignid == 15) or (campaignid > 21)) {
        for k, v in UserDetails.details.game_instances {
            if (v.game_instance_id == ActiveInstance) {
                campaignid := campaignid "a" JSON.stringify(v.defines.adventure_defines[1].requirements[1].adventure_id)}}}kleholink := kleholink campaignid "/"for k, v in UserDetails.details.game_instances {
        if (v.game_instance_id == ActiveInstance) {
...
IncompletePatron(patronid)
{
    ...
    for k, v in campaignresults.campaigns {
        for k2, v2 in v.available_patron_adventure_ids {
            for k3, v3 in v2 {
                if ((k3 == patronid) && (v3[1] == 1))availablelist.push(k2)}}for k2, v2 in v.completed_patron_adventure_ids {
            for k3, v3 in v2 {
                if ((k3 == patronid) && (v3[1] == 1))completelist.push(k2)}}if (availablelist[1]) {
            campaignvariants := campaignvariants "`n" CampaignFromID(v.campaign_id) "- "
        }
        for k2, v2 in availablelist {
            campaignvariants := campaignvariants v2 ", "
...
AdventureList()
{
    ...
    while (count < freeplayids.Count()) {
        ...
        tempname := JSON.stringify(freeplaynames[count])testvar := testvar tempname ", "count += 1}StringTrimRight, testvar, testvar, 1testvar := testvar "}"FileDelete, advdefs.jsonFileAppend, %testvar%, advdefs.jsonMsgBox % "advdefs.json saved to file."return}GearReport()
{
    totalgearlevels := -1
    totalgearitems := -1
    totalcorelevels := -1
...
ScrollBox(String := "", Options := "", Label := "")
{
    ...
    DetectHiddenWindows, % (Setting_A_DetectHiddenWindows := A_DetectHiddenWindows) ? "On" :
        ...
        if Label
        {
            ...
            if RegExMatch(RegExReplace(Options, "\{.*}"), "i)d")Gui, %Gui_Label%:Destroyelse if WinExist("ahk_id " Win_Hwnd) {
                if String
                    GuiControl, , %Gui_Hwnd%, %String%
                WinGetPos, WinX, WinY, WinW, WinH
                if RegExMatch(Options, "i)x(\d+)", Match)WinX := Match1if RegExMatch(Options, "i)y(\d+)", Match)WinY := Match1if RegExMatch(Options, "i)w(\d+)", Match)WinW := Match1if RegExMatch(Options, "i)h(\d+)", Match)WinH := Match1Winmove, ahk_id %Win_Hwnd%, , WinX, WinY, WinW, WinHif RegExMatch(Options, "i)h(?!\d)", Match)Gui, %Gui_Label%:Hideif RegExMatch(Options, "i)s", Match)Gui, %Gui_Label%:ShowDetectHiddenWindows, %Setting_A_DetectHiddenWindows%SetWinDelay, %Setting_A_WinDelay%return}} else {
        Gui_Index ++
    ...
    if (!String and !Options) {
        if Label
        {
            ...
            for key, element in Gui_ListGui, %key%:DestroyGui_List := {}}DetectHiddenWindows, %Setting_A_DetectHiddenWindows%SetWinDelay, %Setting_A_WinDelay%return}Gui %Gui_Label%:DefaultGui +LabelAllGuiAdjust1 := 10ButtonPushed := -2if RegExMatch(Options, "i)f(\d+)", Match) {
        Gui, Font, s%Match1%
        Adjust1 := Match1
    ...
    if !(Options ~= "i)w(?!\d)")
    Opt .= "+0x300000 -wrap "
    if RegExMatch(Options, "i)h(\d+)", Match)Opt .= "h" Match1 " ", Control := trueif RegExMatch(Options, "i)w(\d+)", Match)Opt .= "w" Match1 " ", Control := trueif (Options ~= "i)c")Opt .= "center "if (Options ~= "i)l")Opt .= "left "if (Options ~= "i)r")Opt .= "right "Loop {
        Pos ++
        if (Pos := RegExMatch(Options, "i)t(\d+)", Match, Pos))Opt .= "t" Match1 " "} until !PosOpt_Show := "AutoSize "if RegExMatch(Options, "i)x(\d+)", Match)Opt_Show .= "x" Match1 " "if RegExMatch(Options, "i)y(\d+)", Match)Opt_Show .= "y" Match1 " "if Control {
        Gui, Add, Edit, % Opt
        GuiControl, , %Gui_Hwnd%, %String%
    } else {
    ...
    if ((Options ~= "i)p(?!\d)") or (!(Options ~= "i)p") and Button))while (ButtonPushed = -2)Sleep 50else if RegExMatch(Options, "i)p(\d+)", Match) {
        TimeEnd := A_TickCount + Match1
        while (A_TickCount < TimeEnd and ButtonPushed = -2)Sleep 50Gui_List.Delete(Gui_Label)Gui, Destroy}DetectHiddenWindows, %Setting_A_DetectHiddenWindows%SetWinDelay, %Setting_A_WinDelay%Gui, 1:Defaultreturn ButtonPushedAllGuiSize:Resize_Gui_Hwnd := Gui_List[A_Gui]if Button {
            if (Button = 1) {
